...
function App() {
  ...
  // NEW: detect “I don’t know / no context” answers from assistant
  const looksLikeNoAnswer = (text: string) => {
    const lower = text.toLowerCase();
    return (
      lower.includes("i'm not sure about that") ||
      lower.includes("the provided context does not include") ||
      lower.includes("i'm not sure based on the documents") ||
      lower.includes("i do not have enough information") ||
      lower.includes("i don't have enough information") ||
      (lower.includes("consult a healthcare professional") &&
        lower.includes("for accurate") &&
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Strip sources if the assistant is effectively saying “I don't know”
      const sources =
        usedKb && !looksLikeNoAnswer(res.answer) ? res.source_documents : [];

      addMessage("assistant", res.answer, sources);
    } catch (e) {
      addMessage(
        "assistant",
        "⚠️ There was an error contacting the backend. Please check your connection and try again.",
  ...
  const handleDownloadChat = () => {
    ...
    conv.messages.forEach((m) => {
      ...
      if (
        m.sourceDocuments &&
        m.sourceDocuments.length > 0 &&
        !looksLikeNoAnswer(m.content)
      ) {
        lines.push("\nSources:");
        m.sourceDocuments.forEach((doc, idx) => {
          lines.push(`  ${idx + 1}. ${doc.metadata?.source || "Document"}`);
